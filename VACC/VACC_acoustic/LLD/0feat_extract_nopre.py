...
# -----------------
def_wav = '/home/jay_kejriwal/Fisher/Processed/VACC_audio_resampled/20171121A_Calendar_01.wav'
config_path = '/home/jay_kejriwal/Fisher/Programs/LLD/emobase2010_haoqi_revised.conf'
out_dir = '/home/jay_kejriwal/Fisher/Processed/Embeddings/VACC_audio_LLD/'
transcript_dir='/home/jay_kejriwal/Fisher/Processed/VACC_text'
feat_dir = '/home/jay_kejriwal/Fisher/Programs/VACC/LLD/raw_features/'
...
if extract:
	...
	sample_rate = subprocess.run(cmd_check_sample_rate, shell  = True)
...
if extract:
	transcript = transcript_dir +'/'+ basename(INPUT_audio).split(ext)[0].split('--')[1] + '.txtturns_revised.txt'
	#basename(Trancript_audio).split(ext)[0].split('--')[1] + '.txt'
else:
	transcript = transcript_dir +'/'+ basename(INPUT_audio).split(ext)[0] + '.txtturns_revised.txt'
trans = open(transcript).readlines()
# pdb.set_trace()
for line in trans:
	if line!='\n':
		if line[0] !='#':
			start, stop, spk, text = line.split('\t')
