...
# Program adapted from Md. Nasir https://github.com/nasir0md/unsupervised-learning-entrainment
# ------------------------------------------------------------------------
...
# -----------------
#Define path for wav file
def_wav = r'D:\Jay\DNN\CGC\jkdata\session_01\s01.cards.1.wav'
#Define path for transcription
transcript_dir=r'D:\Jay\DNN\CGC\jkdata\session_01\s01.cards.1.IPU_txt'
#Define temporary path for features pre-processing 
#This path will save raw features extracted using opensmile toolkit
feat_dir = r'D:\Jay\DNN\CGC\Pre-processing\IPU_raw_feats'
#Define path for OPENSMILE config file
config_path = r'D:\Jay\DNN\CGC\emobase2010_haoqi_revised.conf'

#Define output path
out_dir = r'D:\Jay\DNN\CGC\Pre-processing\IPU_feats'


# python 1.1Extarct_LLD_feat.py --audio_file wav/fe_03_00001.sph -- --openSMILE_config emobase2010_haoqi_revised.conf --output_path feats
...
Trancript_audio  = args.audio_file
...
#---------------------------------------------------------------------
opensmilepath=r'C:\Opensmile\opensmile-3.0-win-x64\bin'
if extract:
	not_wav = False
	...
	cmd_feat = opensmilepath + './SMILExtract '+ '-nologfile -C %s -I %s -O %s' %(CONFIG_openSMILE, INPUT_audio, csv_file_name)
...
if extract:
	transcript = os.path.splitext(Trancript_audio)[0] + '.txt'
else:
	transcript = basename(Trancript_audio).split(ext)[0] + '.txt'
trans = open(transcript).readlines()
# pdb.set_trace()
for line in trans:
	if line!='\n':
		if line[0] !='#':
			start, stop, spk, file = line.split('\t')
